$(document).ready(function () {
    ...
    //-------------------------------------ОТ СЕРВИСА К МАСТЕРУ-------------------------------------
    $('#nextBtnToMaster').on('click', function () {
        ...
        popup.animate({ scrollTop: 0 }, {
            ...
            complete: function () { // Функция завершения
                ...
                $.ajax({
                    ...
                    method: 'GET',
                    data: { id_service: selectedServiceId },
    ...
    $('#nextBtnToDateTime').on('click', function () {
        ...
        popup.animate({ scrollTop: 0 }, {
            ...
            complete: function () {
                ...
                // Получить id_service и id_master выбранных элементов
                var id_service = $('.service__item input[type="radio"]:checked').val();
                var id_master = $('.master__item input[type="radio"]:checked').val();
                var selectedService = $('input[name="service"]:checked');
                var duration_service_selected = selectedService.data('duration');

                // Отправить id_service и id_master на следующий шаг
                // Здесь можно использовать AJAX для отправки данных на сервер или другие нужные действия
                console.log('id_service:', id_service);
                console.log('id_master:', id_master);
                console.log('duration_service_selected:', duration_service_selected);
    ...
    $.ajax({
        url: '../handlers/getServicesTypes.php',
        method: 'GET',
        success: function (data) {
            var options = '';
            var selectedOption = '';
    ...
    function loadServices(id_service_type) {
        if (id_service_type) {
            $.ajax({
                url: '../handlers/getServicesByType.php',
                method: 'GET',
